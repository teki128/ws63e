...
enum spooky_decoder_init_res spooky_decoder_init(struct spooky_decoder *dec,
...
#define STATE(NAME) static int NAME(struct spooky_decoder *dec, bool bit)
...
static void append_to_ring_buffer(struct spooky_decoder *dec, uint8_t offset)
{
    ...
    buf[dec->index & RING_BUF_MASK] = (dec->index == 0 ? MAX_POSSIBLE_DELAY : dec->ticks - offset);
    if (DEBUG)
...
static int sink_bit_with_cb(struct spooky_decoder *dec, bool bit, byte_cb *cb, bool save_ticks)
{
    int res = 0;
    LOG("sink_bit, interval %u, ticks %u, bit %u, pre_ticks %u, last %d, accum 0x%02x\n", dec->interval, dec->ticks,
        bit, dec->pre_ticks, dec->last, dec->bit_accum);
...
STATE(step_length)
{
    return sink_bit_with_cb(dec, bit, length_byte_cb, true);
}

/* Read a checksum byte. */
STATE(step_chksum)
{
    return sink_bit_with_cb(dec, bit, chksum_byte_cb, true);
}

/* Read the data payload. */
STATE(step_payload)
{
    return sink_bit_with_cb(dec, bit, payload_byte_cb, false);
}
